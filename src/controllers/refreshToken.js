...
const refreshToken = async (req, res) => {
    ...
    const dbResp = await dbQuery('SELECT refresh.token, refresh.expiration, users.username FROM refresh INNER JOIN users ON users.id = refresh.user_id WHERE refresh.token = ?', [refToken])
